...
public class Misplay extends BaseCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        addToBot(new DamageAction(target, new DamageInfo(p, 2, DamageInfo.DamageType.NORMAL),
        ...
        if (target == p) {
            ...
            if (numMonsters == 1 || rand == 0) {
                ...
                while (r.id != target.id && numMonsters > 1) {
                    r = AbstractDungeon.getMonsters().getRandomMonster((AbstractMonster) null, true,
                            AbstractDungeon.cardRandomRng);
                }
                addToBot(new DamageAction(r, new DamageInfo(p, this.damage, DamageInfo.DamageType.NORMAL),
                        AbstractGameAction.AttackEffect.NONE));
            }
